...
const Deposit: React.FC = () => {
  ...
  useEffect(() => {
    const savedData = localStorage.getItem("depositId");
    if (savedData) {
      const { id, expirationTime } = JSON.parse(savedData);
  ...
  const saveDepositIdToLocalStorage = (id: string, expirationMinutes: number) => {
    const expirationTime = new Date().getTime() + expirationMinutes * 60 * 1000;
    const data = { id, expirationTime };
    localStorage.setItem("depositId", JSON.stringify(data));
  };
      ...
      case 0:
        ...
        try {
          ...
          if (response.status === 201 || response.status === 200) {
            ...
            saveDepositIdToLocalStorage(response.data.depositId, 60);
            setCurrentStep(1);
          }
        } catch (error) {
          console.log(error);
