...
import ListContacts from "../profile/ListContacts";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "../ui/select";

// icons
import { LucideArrowLeft, PencilLine, UserCircle2 } from "lucide-react";
...
interface CreateStepsProps {
  t: (key: string) => string;
  type: "withdraw" | "redeem" | "contact";
  addressDestination: `0x${string}` | null;
  setType: (type: "withdraw" | "redeem" | "contact") => void;
...
const titles = (type: "redeem" | "withdraw" | "contact", status: RedeemStatus | null) => ({
  0: "createWithdrawOrder",
  1: type === "redeem" ? "redeemStep1" : type === "contact" ? "selectContact" : "transferStep1",
  2:
    type === "redeem"
    ...
    children: (
      <form id={formIds.createOrder} onSubmit={handleSubmit} className="flex flex-col gap-2">
        ...
        <button
          className={cn(
            "flex w-full items-center gap-2.5 rounded-md p-3 border-2 border-black shadow-brutalist-sm",
            type === "contact" ? "bg-[#F0FCFF]" : "bg-white"
          )}
          type="button"
          onClick={() => setType("contact")}
        >
          <Image
            src="/images/app/contact-vector.svg"
            alt="contact-vector"
            width={52}
            height={52}
            className=""
          ...
          <div className="flex flex-col items-start justify-center relative">
            <p className="font-bold text-base">{t("contacts")}</p>
            <p className="text-base">{t("sendCLPD")}</p>
          </div>
        </button>
      </form>
    ),
  ...
  },
  {
    ...
    children:
      type === "redeem" ? (
        ...
        <form id={formIds.contact} onSubmit={handleSubmit} className="flex flex-col gap-3">
          <ListContacts
            contacts={contacts}
            withdrawPage
    ...
    formId:
      type === "redeem" ? formIds.reedem : type === "withdraw" ? formIds.transfer : formIds.contact,
  },
  {
    step: 2,
    title:
...
const Withdraw: React.FC = () => {
   ...
   */
  const [type, setType] = useState<"redeem" | "withdraw" | "contact">("redeem");
  ...
  const userAddress = user.address || "0x0000000000000000000000000000000000000000";
  ...
  const withdrawType = params.get("type");

  useEffect(() => {
    if (withdrawType === "contact") {
      setType("contact");
    }
  }, [withdrawType]);
  ...
  const fetchContacts = async (phoneNumber: string) => {
    ...
    const contacts = await axios.get<Contact[]>(`/api/contacts?phoneNumber=${phoneNumber}`);
    if (contacts.status === 200) {
      setContacts(contacts.data);
    }
    // const mockContacts: Contact[] = [
    //   { id: "1", name: "Juan Pérez", phoneNumber: "123-456-7890" },
    //   { id: "2", name: "María López", phoneNumber: "234-567-8901" },
    //   { id: "3", name: "Carlos García", phoneNumber: "345-678-9012" },
    //   { id: "4", name: "Ana Martínez", phoneNumber: "456-789-0123" },
    //   { id: "5", name: "Luis Rodríguez", phoneNumber: "567-890-1234" },
    //   { id: "6", name: "Sofía Hernández", phoneNumber: "678-901-2345" },
    //   { id: "7", name: "Diego Sánchez", phoneNumber: "789-012-3456" },
    //   { id: "8", name: "Laura Torres", phoneNumber: "890-123-4567" },
    //   { id: "9", name: "Pedro Ramírez", phoneNumber: "901-234-5678" },
    //   { id: "10", name: "Claudia Jiménez", phoneNumber: "012-345-6789" },
    // ];
      ...
      {currentStep !== 2 && (
        <CardFooter>
          <Button
            ...
            disabled={errorFields.length > 0 || withdrawAmount === ""}
          >
            {!loading ? (
              (() => {
                switch (currentStep) {
