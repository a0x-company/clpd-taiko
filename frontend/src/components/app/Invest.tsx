          ...
          <div className="flex items-center gap-4 relative">
            ...
            <div className="absolute bottom-0 right-0 flex items-end gap-2">
              <button
                ...
                className={cn(
                  "bg-white border-2 border-black rounded-full p-1 text-sm font-bold hover:bg-black/5 transition-colors",
...
const Invest: React.FC = () => {
  ...
  const handleSwitchTokens = () => {
    setCurrencyInvest((prev) => {
      const newCurrency = prev === "CLPD" ? "USDC" : "CLPD";
      const isSufficientBalance =
        newCurrency === "CLPD" ? clpdBalanceFormatted : usdcBalanceFormatted;

      if (Number(amount) > Number(isSufficientBalance)) {
        setAmount(isSufficientBalance);
        setMax(true);
      } else {
        setMax(false);
      }
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    switch (currentStep) {
      case 0:
        ...
        try {
          setCurrentStep(1);
          const response = await axios.post(`/api/invest/${currencyInvest.toLowerCase()}`, {
            investAmount: amount,
          });
