...
import { BurnStatus, DepositStatus } from "@internal/deposits";
...
const BURN_CHECK_PERIOD = 2 * 60 * 60 * 1000; // 2 horas en milisegundos
...
export function getVaultBalanceHandler(ctx: VaultContext) {
  return async (req: Request, res: Response) => {
    ...
    try {
      ...
      // Verificar depósitos pendientes vencidos
      const pendingDeposits = await ctx.depositService.getPendingDeposits();
      const doceHoras = 12 * 60 * 60 * 1000;
      const ahora = Date.now();
      console.log(`Depositos pendientes: ${pendingDeposits.length}`);
      console.log(`Los depositos son: ${JSON.stringify(pendingDeposits)}`);
      const depositosVencidos = pendingDeposits.filter(deposit => {
        return (ahora - deposit.createdAt) > doceHoras;
      });

      if (depositosVencidos.length > 0) {
        const updates = depositosVencidos.map(deposit => ({
          id: deposit.id,
          data: {
            status: DepositStatus.REJECTED,
            rejectionReason: "Depósito rechazado automáticamente por tiempo de espera excedido (12 horas)",
            updatedAt: ahora
          }
        }));

        await ctx.depositService.storage.updateMultipleDeposits(updates);
        console.log(`🕒 ${depositosVencidos.length} depósitos rechazados por tiempo excedido`);
      }

      if (isStorageRoute) {
        balance = await ctx.storageService.getCurrentBalance();
        console.log("💾 Balance obtenido del almacenamiento.");
      } else {
        // Verificar si hay depósitos accepted_not_minted antes de hacer scraping
        const notMintedDeposits = await ctx.depositService.getDepositsByStatus(DepositStatus.ACCEPTED_NOT_MINTED);
        const pendingDeposits = await ctx.depositService.getPendingDeposits();
        const recentBurns = await ctx.depositService.getBurnRequestsByStatus(BurnStatus.BURNED);
        
        const hasRecentBurns = recentBurns.some(burn => 
          (ahora - burn.updatedAt) <= BURN_CHECK_PERIOD
        );

        const hasRecentPendingDeposits = pendingDeposits.some(deposit => 
          (ahora - deposit.createdAt) <= doceHoras
        );

        if (notMintedDeposits.length > 0 || hasRecentBurns || hasRecentPendingDeposits) {
          console.log(`🔄 Realizando scraping debido a: ${
            notMintedDeposits.length > 0 ? 'depósitos pendientes de minteo' : 
            hasRecentPendingDeposits ? 'depósitos pendientes recientes' : 
            'quemas recientes'
          }`);
          balance = await attemptScraping(ctx);
        } else {
          balance = await ctx.storageService.getCurrentBalance();
          console.log("💾 No hay actividad reciente, usando balance almacenado.");
        }
      }

      if (balance === null) {
        throw new Error("❌ CRÍTICO: No hay balance disponible.");
      }

      res.status(200).json({ balance });
      console.log(`📊 Balance devuelto: ${balance}`);
    } catch (error: unknown) {
